...
function Initials({ name = "GU", bg = "#FEF2F2" }) {
  const text = String(name || "GU").slice(0, 2).toUpperCase();
  return (
    <div
      className="flex h-10 w-10 items-center justify-center rounded-full text-sm font-semibold"
      style={{ background: bg, color: TOKENS.accent }}
    >
      {text}
    </div>
  );
      ...
      <div className="w-full flex justify-center">
        <div className="relative inline-flex items-center gap-8">
          ...
          <button
            ...
            type="button"
            onClick={() => onChange("general")}
            className={`text-xl font-semibold flex items-center gap-2 transition-all ${
              isGeneral ? "text-gray-900" : "text-gray-400 hover:text-gray-600"
            } ${isGeneral ? "animate-[wiggleScale_360ms_ease-in-out]" : ""}`}
            aria-pressed={isGeneral}
          >
            <span aria-hidden>💬</span>
          ...
          <button
            ...
            className={`text-xl font-semibold flex items-center gap-2 transition-all ${
              !isGeneral
                ? "text-gray-900 animate-[wiggleScale_360ms_ease-in-out]"
...
function FreeSearchBar({ value, onSubmit, onReset, compact }) {
  ...
  return (
    <form
      onSubmit={handle}
      className={`mx-auto ${compact ? "mt-1" : "mt-5"} w-full max-w-[860px] rounded-2xl shadow-sm bg-white border border-gray-200 overflow-hidden hover:shadow-md transition-shadow`}
    >
      <div className="grid grid-cols-[1fr_auto] items-stretch">
        <div className="px-5 py-3.5">
          <div className="text-xs font-medium text-gray-500 mb-1">Keyword</div>
          <input
            value={local.q || ""}
            onChange={(e) => setLocal({ q: e.target.value })}
            placeholder="Search posts…"
        ...
        <div className="flex items-center gap-2 px-4 border-l border-gray-100">
          ...
          <button
            ...
            className="px-4 py-2 rounded-lg text-sm font-medium text-white shadow-sm hover:shadow transition-all"
          >
            Search
          </button>
        </div>
...
function AcademicSearchBar({ value, onSubmit, onReset, compact }) {
                ...
                {TYPE_ROWS.map((row) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl transition text-left
                      ${ (local.type || "question") === row.key
                        ? "bg-[#FEF2F2] border-2 border-[#FF7A70]"
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-gray-500 truncate">{row.desc}</div>
                    </div>
                  </button>
                ))}
              </div>
  ...
  return (
    <div className={`mx-auto ${compact ? "mt-1" : "mt-5"} w-full max-w-[960px]`} ref={anchorRef}>
      <form
        onSubmit={handleSubmit}
        className="rounded-2xl shadow-sm bg-white border border-gray-200 overflow-hidden hover:shadow-md transition-shadow"
      >
        <div className="grid grid-cols-[1.2fr_0.9fr_auto] items-stretch">
          ...
          <button
            type="button"
            onClick={() => { measure(); setOpen(true); setStep("type"); }}
            className="px-5 text-left py-3.5 border-r border-gray-100 hover:bg-gray-50 transition-colors"
          >
...
function CollapsedGeneral({ value, onSubmit, onReset, onOpenExpanded, right }) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 flex items-center justify-between gap-3">
      <form
        onSubmit={submit}
        className="flex-1 max-w-[720px] rounded-full border border-gray-300 bg-white shadow-sm overflow-hidden hover:shadow-md transition-shadow"
      >
        <div className="flex items-center">
          <div
            className="flex items-center gap-2 pl-4 pr-3 py-2.5 flex-1 cursor-text"
            onClick={onOpenExpanded}
            title="Open expanded search"
          >
            <span className="text-base">💬</span>
          ...
          <div className="flex items-center gap-2 pl-3 pr-3">
            <button
              type="button"
              onClick={() => onReset?.()}
              className="px-2 py-1.5 rounded-full text-xs font-medium text-gray-600 hover:bg-gray-100 transition-colors"
              title="Reset"
            >
              Reset
            </button>
            <button
              type="submit"
              style={{ backgroundColor: TOKENS.accent }}
              className="px-3 py-1.5 rounded-full text-xs font-medium text-white shadow-sm hover:shadow transition-all"
              title="Search"
            >
              Search
            </button>
                ...
                {TYPE_ROWS.map((row) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-left transition-colors
                      ${ (local.type || "question") === row.key
                        ? "bg-[#FEF2F2] border border-[#FF7A70]"
      ...
      >
        <div className="flex items-center">
          ...
          <button
            type="button"
            onClick={() => { measure(); setOpen(true); setStep("type"); }}
            className="px-3 py-2.5 text-xs font-medium text-gray-700 min-w-[140px] text-center hover:bg-gray-50 transition-colors"
            title="Choose type"
          >
            {local.type === "seeking" ? "Seeking" : "General Question"}
          </button>
  ...
  const RightUserArea = (
    <>
      <button
        type="button"
        onClick={() => { setShowNoti((v) => !v); setShowProfile(false); }}
        className="relative ml-1 px-3 py-2 text-base rounded-lg hover:bg-[#FEF2F2] transition-colors"
        aria-label="Notifications"
        title="Notifications"
      >
        <span className="text-xl leading-none">🔔</span>
        {count > 0 && (
          <span
            className="absolute -top-1 -right-1 inline-flex items-center justify-center min-w-[18px] h-[18px] px-1 rounded-full text-xs font-medium text-white"
            style={{ backgroundColor: TOKENS.accent }}
          >
            {count}
          </span>
      ...
      {user?.email ? (
        ...
        <>
          <Link to="/login" className="px-4 py-2 text-sm font-medium text-gray-700 hover:text-gray-900 hover:bg-gray-50 rounded-lg transition-colors">
            Log in
          </Link>
          <Link to="/register" className="px-4 py-2 text-sm font-medium text-gray-700 hover:text-gray-900 hover:bg-gray-50 rounded-lg transition-colors">
            Register
          </Link>
        </>
      )}
    ...
    <div className="min-h-screen flex bg-[#F8F9FA]">
      ...
      <aside className="hidden md:flex md:w-64 flex-col border-r border-gray-200 bg-white">
        <div className="p-6 border-b border-gray-200">
          ...
          {school && <div className="mt-1.5 text-xs text-gray-500 font-medium truncate">{String(school)}</div>}
        </div>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        {/* 헤더: compact일 땐 숨김(General/Academic 모두) */}
        {!isCompact && (
          <header className="w-full bg-white border-b border-gray-200 mt-2 md:mt-3">
            <div className="mx-auto max-w-7xl px-6 h-16 flex items-center justify-end gap-3">
              {RightUserArea}
            </div>
          </header>
        )}
        ...
        {/* 전역 스티키 바 */}
        <div className="sticky top-0 z-30 border-b border-gray-200 bg-white/95 backdrop-blur-sm transition-all">
          <div className={`${isCompact ? "py-2" : "py-4"} transition-all`}>
            {isCompact ? (
              // ✅ 컴팩트 모드: 대시보드 외 페이지에서도 항상 표시
              active === "general" ? (
                <CollapsedGeneral
        ...
        {openSearch && isDashboard && (
          <div
            className="fixed inset-0 z-40 bg-black/40 backdrop-blur-sm flex items-start justify-center pt-16"
            onClick={() => setOpenSearch(false)}
          >
            <div className="w-full max-w-4xl px-4" onClick={(e) => e.stopPropagation()}>
  ...
  return (
    <div className="relative">
      <button
        type="button"
        onClick={() => setShowProfile((v) => !v)}
        className="flex items-center gap-2.5 rounded-lg px-3 py-2 hover:bg-[#FEF2F2] transition-colors"
      >
        <Initials name={nickname} />
        <span className="hidden sm:inline text-sm font-medium text-gray-900">{nickname}</span>
        <svg className={`h-4 w-4 text-gray-500 transition-transform ${showProfile ? "rotate-180" : ""}`} viewBox="0 0 20 20" fill="currentColor">
          <path d="M5.23 7.21a.75.75 0 011.06.02L10 10.94l3.71-3.71a.75.75 0 111.06 1.06l-4.24 4.25a.75.75 0 01-1.06 0L5.21 8.29a.75.75 0 01.02-1.08z" />
        </svg>
      </button>
      ...
      {showProfile && (
        <>
          <div className="fixed inset-0 z-40" onClick={() => setShowProfile(false)} />
          <div className="absolute right-0 z-50 mt-2 w-56 rounded-xl border border-gray-200 bg-white shadow-xl">
            ...
            <div className="border-t border-gray-100 p-1.5">
              <button
                onClick={onLogout}
                style={{ color: TOKENS.accent }}
                className="w-full rounded-lg px-3 py-2 text-left text-sm font-medium hover:bg-[#FEF2F2] transition-colors"
              >
                Log out
              </button>
            </div>
...
function MenuItem({ to, children, onClick }) {
  return (
    <Link
      to={to}
      onClick={onClick}
      className="block rounded-lg px-3 py-2 text-gray-700 hover:bg-gray-50 transition-colors font-medium"
    >
      {children}
    </Link>
  );
