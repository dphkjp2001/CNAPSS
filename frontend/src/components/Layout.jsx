...
export default function Layout() {
  ...
  const { items, count, markAllAsRead } = useNotificationsPolling(
    user?.email,
    5000,
    60000,
    5
      ...
      {showScheduleMenu && (
        ...
        >
          <div className="bg-white rounded-xl p-6 shadow-lg" onClick={(e) => e.stopPropagation()}>
            <div className="text-sm text-gray-700">Schedule features coming soon.</div>
          </div>
        </div>
      )}
