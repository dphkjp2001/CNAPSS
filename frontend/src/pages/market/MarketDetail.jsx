...
export default function MarketDetail() {
  ...
  // Load item (public for guests, protected for authed)
  useEffect(() => {
    let mounted = true;
    (async () => {
      try {
        let data;
        if (token) {
          data = await getItem({ school, token, id });
        } else {
          data = await getPublicMarketItem({ school, id });
          // normalize minimal fields to match UI usage
          data = {
            ...data,
            sellerNickname: data.sellerNickname || "Unknown",
        ...
        console.error(e);
        setErrorMsg(e?.payload?.message || e?.message || "Failed to load the item.");
      } finally {
  ...
  // Check if request already sent (and get conversationId) — only when logged in & not owner
  useEffect(() => {
    let mounted = true;
    (async () => {
              ...
              <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                ...
                {alreadySent && user ? (
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    />
                    <button
                      onClick={onClickSend} // 🔑 gate opens here if not logged in
